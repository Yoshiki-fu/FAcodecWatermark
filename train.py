...
def main(args):
    ...
    config = yaml.safe_load(open(config_path))      # 設定を辞書形式に読み込む
    ...
    batch_size = config.get('batch_size', 10)       # 4
    batch_length = config.get('batch_length', 120)
    device = accelerator.device

    epochs = config.get('epochs', 200)      # 1000
    log_interval = config.get('log_interval', 10)
    saving_epoch = config.get('save_freq', 2)       # 1
    ...
    discriminator_iter_start = config['loss_params'].get('discriminator_iter_start', 0)     # 2000
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            if iters % (log_interval * 1000) == 0 and accelerator.is_main_process:
                with torch.no_grad():
                    ...
                    style2 = model.quantizer.timbre_linear(timbre2).unsqueeze(2)     # style2 = model.quantizer.module.timbre_linear(timbre2).unsqueeze(2)  (B, 2d, 1)
                    gamma, beta = style2.chunk(2, 1)  # (B, d, 1)
                    x = x.transpose(1, 2)
                    x = model.quantizer.timbre_norm(x)       # x = model.quantizer.module.timbre_norm(x)
                    x = x.transpose(1, 2)
                    x = x * gamma + beta
