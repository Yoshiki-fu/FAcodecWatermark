...
def main(args):
    ...
    for epoch in range(start_epoch, hp.epoch):
        ...
        for i, batch in enumerate(train_loader):
            ...
            loss_gen_all = waveform_loss * 1.0 + mel_loss * 2.0 + stft_loss * 1.0 + loss_feature * 1.0 + loss_g * 1.0 + vad_loss * 1.0 + sim_loss * 2.0 + msg_loss * 1.0
            loss_gen_all.backward()

            grad_norm_g3 = torch.nn.utils.clip_grad_norm_(watermark_model.quantizer.parameters(), 1.0)
            grad_norm_g4 = torch.nn.utils.clip_grad_norm_(extracter.quantizer.parameters(), 1.0)
